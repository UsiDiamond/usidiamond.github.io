...
for (const feature of features) {
  for (const scenario of feature.elements) {
    const failed = scenario.steps.some((s) => s.result && s.result.status === 'failed');
    const skipped =
      !failed && scenario.steps.some((s) => s.result && s.result.status === 'skipped');
...
for (const feature of features) {
  ...
  const subtitle = featureFailed ? ` — ${failed} failed` : ` — ${passed} passed`;
  ...
  for (const scenario of feature.elements) {
    ...
    const skipped =
      !failedStep && scenario.steps.some((s) => s.result && s.result.status === 'skipped');

    let statusCell;
    if (failedStep) {
      const errMsg = truncate(failedStep.result.error_message);
      statusCell = errMsg ? `❌ Failed<br><sub><code>${esc(errMsg)}</code></sub>` : '❌ Failed';
    } else if (skipped) {
    ...
    out.push(`| ${esc(scenario.name)} | ${statusCell} | ${durationCell} |`);
  }
...
if (ARTIFACT_URL) {
  out.push(`📥 [Download HTML report with screenshots](${ARTIFACT_URL})`);
} else {
  out.push(
